package cn.swu.bs.user.service;
...
import cn.swu.bs.email.Email;
import cn.swu.bs.user.dao.UserDao;
...
public class UserService {
	...
	public void activation(String code) throws UserException {
		//1通过激活码查询用户
		//2。User为null,无效激活码，抛出异常，给出异常信息
		//3。查看用户状态是否为true，如果为true，抛出异常，给出异常信息
		//4。修改用户状态，
	...
	public void regist(User user) {
		...
		String content = MessageFormat.format(prop.getProperty("content"),user.getActivationCode());//替换{0}{1}
		List<String> recipients = new ArrayList<String>();
		recipients.add(user.getEmail());
		Email sendMailBySSL = new Email(host, port,from,pass,recipients, subject, content);
